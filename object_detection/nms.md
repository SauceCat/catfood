...
def nms(np.ndarray[np.float32_t, ndim=2] dets, np.float thresh):
    ...
    for _i in range(ndets):
        i = order[_i]
        # if it has already been suppressed by other boxes
        if suppressed[i] == 1:
            continue
        keep.append(i)
        ix1 = x1[i]
        ...
        for _j in range(_i + 1, ndets):
            ...
            # suppress when iou is above certain threshold
            if ovr >= thresh:
                suppressed[j] = 1
...
reference: https://paperswithcode.com/method/soft-nms

![img](images/soft_nms_1.png)

![img](images/soft_nms_2.png)
